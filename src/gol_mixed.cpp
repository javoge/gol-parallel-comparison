// ============================================================
// Juego de la Vida - Ejecucion Mixta: MPI + OpenMP
// Trabajo Final - Programacion Paralela y Distribuida
// ============================================================
// Combina distribucion de memoria (MPI) con paralelismo
// de datos en CPU (OpenMP) dentro de cada nodo/proceso.
// Cada proceso MPI usa multiples hilos OpenMP para procesar
// su porcion local de filas.
// ============================================================
#include <iostream>
#include <vector>
#include <fstream>
...
int main(int argc, char* argv[]) {
    ...
    if (rank == 0) {
        cout << "=== MIXTO (MPI + OpenMP) ===" << endl;
        cout << "Grid: " << ROWS << "x" << COLS << " | Steps: " << STEPS
             << " | Procesos MPI: " << nprocs
             << " | Hilos OpenMP: " << THREADS << endl;
        cout << "Total trabajadores: " << nprocs * THREADS << endl;
    }
    ...
    if (rank == 0) {
        ...
        ofstream out("results_mixed.txt");
        out << "mode,rows,cols,steps,procs,threads,time_s,time_per_step_ms,alive_end" << endl;
        out << "mixed," << ROWS << "," << COLS << "," << STEPS << "," << nprocs << ","
            << THREADS << "," << elapsed << "," << (elapsed / STEPS) * 1000.0
            << "," << total_alive << endl;
        out.close();
    }
